...
namespace congcong.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            congcongEntities db = new congcongEntities();
            if (ModelState.IsValid)
            {
                // 尝试注册用户
                try
                {
                    cc_user ex = new cc_user();
                
                    ex.email = model.UserName;
                    ex.securityQuestion = model.securityQuestion;
                    ex.securityAnswer = model.securityAnswer;
                    ex.name = model.name;
                    ex.publickMessageNum = 0;
                    ex.secretMessageNum = 0;
                    ex.type = "user";
                    ex.frendsNum = 0;
                    //ex.Id = model.Id;                              
                    if (true)
                    {
                        ...
                        ex.id = WebSecurity.GetUserId(model.UserName);
                        db.cc_user.Add(ex);
                        db.SaveChanges();
                        return RedirectToAction("Index", "Home");
